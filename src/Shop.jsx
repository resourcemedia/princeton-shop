    ...
    <div className="grid sm:grid-cols-2 gap-4">
      {products.map((p) => (
        <div key={p.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col">
          {p.image_url ? (
            <img src={p.image_url} alt={p.name} className="w-full aspect-[365/230] object-contain bg-white" />
          ) : (
            <div className="w-full aspect-[365/230] grid place-items-center bg-slate-50 text-slate-300">
              <ImageOff size={28} />
            </div>
          )}
